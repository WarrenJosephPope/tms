...
-- sees exactly one statement and does not hit the
-- "cannot insert multiple commands into a prepared statement" error.
-- Each inner statement is passed to EXECUTE using a distinct
-- dollar-quote tag to avoid collisions with the outer $migrate$ tag
-- and with the $$ tags inside the function bodies.
-- ============================================================
...
BEGIN
  ...
  -- ============================================================
  -- FUNCTION: get_latest_pings_for_company
  ...
  EXECUTE 'ALTER PUBLICATION supabase_realtime ADD TABLE loads';
