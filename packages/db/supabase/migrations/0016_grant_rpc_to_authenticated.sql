...
--
-- Wrapped in a single DO block so the entire migration is one
-- SQL statement.  This avoids SQLSTATE 42601 ("cannot insert
-- multiple commands into a prepared statement") when applying
-- through Supavisor's extended query protocol.
-- ============================================================
...
BEGIN
  ...
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.place_bid_atomic(UUID, UUID, UUID, NUMERIC, SMALLINT, TEXT) TO authenticated';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.award_load_to_bid(UUID, UUID, UUID) TO authenticated';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.get_my_bid_position(UUID) TO authenticated';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.get_load_active_bid_count(UUID) TO authenticated';
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.auction_started_for_load(UUID) TO authenticated';
  ...
  EXECUTE 'GRANT EXECUTE ON FUNCTION public.get_load_bids_for_shipper(UUID) TO authenticated';
END;
$migration$;
