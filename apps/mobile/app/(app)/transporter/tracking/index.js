...
export default function TransporterTrackingScreen() {
  ...
  const renderTrip = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => router.push(`/(app)/transporter/tracking/${item.id}`)}
        activeOpacity={0.7}
      >
        ...
        <View style={styles.cardRow}>
          ...
          <Text style={styles.meta} numberOfLines={1}>
            {[item.shipper?.name, item.driver?.full_name].filter(Boolean).join(" · ")}
          </Text>
        </View>
      </TouchableOpacity>
    );
